...
export default function TodayLuckyNumber() {
    const [luckyData, setLuckyData] = useState<LuckyNumberData>({
        goldenAnk: "1-6-3-8", // Updated value from your HTML
        finalAnkMessages: []
    });
    ...
    // Load default data
    useEffect(() => {
        const defaultLuckyData: LuckyNumberData = {
            goldenAnk: "1-6-3-8",
            finalAnkMessages: [
                "MILAN MORNING - 0",
                "SRIDEVI - 0",
    ...
    return (
        <div className="rounded-lg shadow-md mx-2 bg-[#fc9] border-4 border-[#ff0016] p-1 mt-2">
            <div className="text-xl font-bold border-2 border-white rounded-[20px] bg-[#ff00a2] p-1">
                <h3 className="text-white italic font-bold text-center" style={{ textShadow: '2px 2px 4px black' }}>{lucknumber.title}</h3>
            </div>
            <div className="flex justify-between items-stretch mt-2 px-2 py-2">
                ...
                <div className="w-[40%] text-center border-r border-red-600 pr-4">
                    <p className="text-xl font-[Helvetica,sans-serif] italic font-bold text-[#001699]">{lucknumber.leftheading}</p>
                    <h4 className="text-2xl font-extrabold text-[#001699]">{luckyData.goldenAnk}</h4>
                </div>

                {/* Final Ank with Marquee */}
                <div className="w-[60%] text-center pl-4">
                    <p className="text-2xl font-[Helvetica,sans-serif] italic font-bold text-[#001699]">{lucknumber.rightheading}</p>
                    ...
                    <style jsx>{`   
                        @keyframes marqueeUpward {
                            ...
                            100% { transform: translateY(-${luckyData.finalAnkMessages.length * 24}px); }
                        }
                    `}</style>
                </div>
            </div>
